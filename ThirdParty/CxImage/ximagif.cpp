  ...
  for (BOOL bContinue = TRUE; bContinue; )
  {
    ...
    if (bPreviousWasNull || ch==0)
    {
      ...
      case ',': // image
        ...
        const rgb_color* pcurpal = TabCol.paleta;  //Current Palette 
...
bool CxImageGIF::DecodeExtension(CxFile *fp)
{
  ...
  bool bContinue = (1 == fp->Read(&fc, sizeof(fc), 1));
  if (bContinue) {
    /* AD - for transparency */
    if (fc == 0xF9)  {
...
bool CxImageGIF::Encode(CxFile * fp, CxImage ** pImages, int pagecount, bool bLocalColorMap, bool bLocalDispMeth)
{
  ...
  if (bLocalDispMeth) {
    ghost.EncodeExtension(fp);
	  ...
	  const BYTE dm = ghost.GetDisposalMethod();
...
void CxImageGIF::EncodeBody(CxFile *fp, bool bLocalColorMap)
{
  ...
  const int InitCodeSize = head.biBitCount <=1 ? 2 : head.biBitCount;
...
int CxImageGIF::GifNextPixel( )
{
  ...
  --CountDown;
  const int r = GetPixelIndex(curx,cury);
...
void CxImageGIF::compressNONE( int init_bits, CxFile* outfile)
{
  ...
  maxcode = (short)MAXCODE(n_bits = g_init_bits);
	const code_int maxmaxcode = (code_int)1 << MAXBITSCODES;
  ...
  a_count=0;
  register long ent = GifNextPixel();

  output( (code_int)ClearCode );

  while ( ent != EOF ) {
	  const register long c = GifNextPixel();
...
short CxImageGIF::decoder(CxFile* file, CImageIterator* iter, short linewidth, int &bad_code_count)
{
   ...
   */
  register short oc = fc = 0;

   /* Allocate space for the decode buffer */
  if ((buf = new BYTE[linewidth + 1]) == NULL) return(OUT_OF_MEMORY);

   /* Set up the stack pointer and decode buffer pointer */
  register BYTE* sp = stack;
  register BYTE* bufptr = buf;
  register short bufcnt = linewidth;
...
{
	const long ymin = max(0,(long)(GetHeight()-imgdesc.t - imgdesc.h));
	const long ymax = GetHeight()-imgdesc.t;
	const long xmin = imgdesc.l;
	const long xmax = min(GetWidth(), (DWORD)(imgdesc.l + imgdesc.w));

	const long ibg2= imgsrc2.GetTransIndex();

  for(long y = ymin; y < ymax; y++){
    for(long x = xmin; x < xmax; x++){
	    const BYTE i2 = imgsrc2.GetPixelIndex(x - xmin, y - ymin);
...
{
	int repmax = count / rle->rl_table_max;
  int leftover = count % rle->rl_table_max;
  int repleft = (leftover ? 1 : 0);
...
{
	unsigned int cost = 0;
	const unsigned int perrep = (nrepcodes * (nrepcodes + 1)) / 2;
  while (count >= perrep){
    cost += nrepcodes;
    count -= perrep;
  }
  if (count > 0){
	  unsigned int n = rle_isqrt(count);
...
{
	const int withclr = 1 + rle_compute_triangle_count(count, rle->max_ocodes);
  if (withclr < count) {
    rle_output(rle->code_clear,rle);
    rle_clear(rle);
    rle_flush_fromclear(count,rle);
