...
wxString getGamePath(bool noSet)
{
	...
	for (size_t i = 0; i<regpaths.count(); i++)
	{
		l = RegOpenKeyEx((HKEY)HKEY_LOCAL_MACHINE, regpaths.at(i).toStdWString().c_str(), 0, KEY_QUERY_VALUE, &key);

		if (l == ERROR_SUCCESS)
		{
			s = sizeof(path);
			l = RegQueryValueEx(key, L"InstallPath", 0, &t, (LPBYTE)path, &s);
			QString spath = QString::fromLatin1(path);
			if (l == ERROR_SUCCESS && wxDir::Exists(spath.toStdWString()) && sNames.contains(spath) == false)
				sNames.push_back(spath);
			RegCloseKey(key);
		}
	}
	...
	if (sNames.size())
	{
		sNames.push_back("Other");
		folder = QInputDialog::getItem(0, QObject::tr("Please select a Path:"), QObject::tr("Path"), sNames);
	}
	// If we found an install then set the game path, otherwise just set to C:\ for now

	if (folder.isEmpty() || folder == "Other")
	{
		folder = gamePath;
		if (folder.isEmpty())
			folder = "C:\\Program Files\\World of Warcraft\\";

		newPath = QFileDialog::getExistingDirectory(0, QObject::tr("Select Your World of Warcraft Directory"), folder, QFileDialog::ShowDirsOnly);

		if (newPath.isEmpty())  // user probably hit cancel
			return newPath.toStdWString();
	}
	else
		newPath = folder;
	if (!newPath.isEmpty() && newPath.end()->toLatin1() != '/')
		newPath.append("/");
	if (!newPath.endsWith("Data/"))
		newPath.append("Data/");
#elif _MAC // Mac OS X
