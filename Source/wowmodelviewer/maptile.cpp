...
void MapChunk::init(MapTile* Mt, GameFile& f, bool bigAlpha)
{
	...
	*/
	...
	while (f.getPos() < lastpos)
	{
		...
		{
			LOG_ERROR << "mcnk chunk initial error, fcc:" << fcc << ", size:" << Size << ", pos: " << f.getPos() <<
				", size:" << f.getSize();
			break;
		}
		...
		else if (strncmp(fcc, "MCLQ", 4) == 0)
		{
			...
			*/
			...
			lq = new Liquid(8, 8, glm::vec3(xbase, waterlevel[1], zbase));
			//lq->init(f);
  ...
  for (ssize_t j=0; j<17; j++) {
    for (size_t i=0; i<((j%2)?8:9); i++) {
      ...
      glm::vec3 v = glm::vec3(xpos+px, zpos+py,-1);
      *ttv++ = v;
    }
  }
