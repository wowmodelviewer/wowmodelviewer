...
ChoiceDialog::ChoiceDialog(CharControl* dest, int type,
	...
	: wxSingleChoiceDialog(parent, message, caption, choices, nullptr, wxCHOICEDLG_STYLE & ~wxCANCEL & ~wxCENTER,
						   wxDefaultPosition)
{
	cc = dest;
	this->type = type;
	...
	: ChoiceDialog(dest, type, parent, message, caption, choices)
{
	...
	m_pattern = new wxTextCtrl(this, ID_FILTER_TEXT, wxEmptyString, wxDefaultPosition, wxDefaultSize,
	                           wxTE_PROCESS_ENTER); // fnmatch: "*" re: ""
...
void FilteredChoiceDialog::OnImportItem(wxCommandEvent& event)
{
	...
	if (dlg->ShowModal() == wxID_OK)
	{
		...
		if (rec.id != 0)
		{
			...
			if (!found)
			{
				// item is not present in current database
				if (rec.model > 0)
				{
	...
	FilteredChoiceDialog(dest, type, parent, message, caption, choices, quality, keepfirst), m_cats(cats)
{
	...
	if (helpmsg)
		topsizer->Prepend(new wxStaticText(this, -1, wxT("Double-click to select only one category")), 0,
		                  wxEXPAND | wxTOP | wxLEFT, 10);
