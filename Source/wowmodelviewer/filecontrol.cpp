...
BEGIN_EVENT_TABLE(FileControl, wxWindow) // model tree
...
static wxString chos[] =
{
	wxT("Models (*.m2)"), wxT("WMOs (*.wmo)"), wxT("ADTs (*.adt)"), wxT("WAVs (*.wav)"), wxT("OGGs (*.ogg)"),
	wxT("MP3s (*.mp3)"),
	wxT("Images (*.blp)"), wxT("Shaders (*.bls)"), wxT("DBCs (*.dbc)"), wxT("DB2s (*.db2)"), wxT("LUAs (*.lua)"),
...
FileControl::FileControl(wxWindow* parent, wxWindowID id)
{
	...
	try
	{
		txtContent = new wxTextCtrl(this, ID_FILELIST_CONTENT, wxEmptyString, wxPoint(10, 10), wxSize(110, 20),
		...
		fileTree = new wxTreeCtrl(this, ID_FILELIST, wxPoint(0, 35), wxSize(400, 580),
		                          ...
		                          wxTR_NO_LINES);
		choFilter = new wxChoice(this, ID_FILELIST_FILTER, wxPoint(10, 620), wxSize(130, -1), std::size(chos), chos);
...
void FileControl::Export(wxString val, int select)
{
	...
	if (select == 1)
	{
		filename = wxFileSelector(wxT("Save..."), wxGetCwd(), fn.GetName(), fn.GetExt(),
		                          fn.GetExt().Upper() + wxT(" Files (.") + fn.GetExt().Lower() + wxT(")|*.") + fn.
		                          GetExt().Lower());
	}
	else
	{
