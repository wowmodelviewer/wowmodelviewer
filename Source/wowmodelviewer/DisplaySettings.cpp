...
#undef ADD_CONTROLS
	...
	top->Add(new wxButton(this, ID_DISPLAY_SETTINGS_APPLY, _("Apply Settings"), wxDefaultPosition, wxDefaultSize, 0),
	         wxSizerFlags()/*.Expand()*/.Border(wxALL, 10).Align(wxALIGN_LEFT | wxALIGN_BOTTOM));
...
void DisplaySettings::OnButton(wxCommandEvent& event)
{
	...
	{
		if ((oglMode->GetSelection() != video.capIndex) && video.GetCompatibleWinMode(
			video.capsList[oglMode->GetSelection()]))
		{
			LOG_INFO << "Graphics display mode changed.  Requires restart to take effect.";
			wxMessageBox(
				wxT("Graphics display settings changed.\nWoW Model Viewer requires restarting to take effect."),
