...
void drawAxis(const glm::vec3& coord, float size, const glm::vec3& xcolor, const glm::vec3& ycolor,
...
void ModelCanvas::Render(wxPaintEvent& WXUNUSED(event))
{
	...
	if (!video.useMasking)
	{
		...
		{
			// draw origin axis
			drawAxis(glm::vec3(0.0, 0.0, 0.0), 1.0f, glm::vec3(1.0, 0.0, 0.0), glm::vec3(0.0, 1.0, 0.0),
			         glm::vec3(0.0, 0.0, 1.0));

			// draw lookAt axis
			drawAxis(camera.lookAt(), 0.5f, glm::vec3(1.0, 0.0, 0.0), glm::vec3(0.0, 1.0, 0.0),
			         glm::vec3(0.0, 0.0, 1.0));
		}
...
inline void ModelCanvas::RenderModel()
{
	...
	else if (lightType == LIGHT_DYNAMIC)
	{
		for (size_t i = 0; i < MAX_LIGHTS; i++)
		{
			...
			{
				glLightfv(GL_LIGHT0 + (GLenum)i, GL_POSITION,
				          glm::value_ptr(g_modelViewer->lightControl->lights[i].pos));
	...
	{
		glLightModelfv(GL_LIGHT_MODEL_AMBIENT, glm::value_ptr(g_modelViewer->lightControl->lights[0].diffuse));
	...
	*/
	...
	if (model_ && (lightType == LIGHT_DYNAMIC))
	{
