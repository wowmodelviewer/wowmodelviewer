	...
	try
	{
		...
		for (std::vector<Attachment*>::iterator it = root->children.begin(); it != root->children.end(); ++it)
		{
			...
			for (std::vector<Attachment*>::iterator it2 = (*it)->children.begin(); it2 != (*it)->children.end(); ++it2)
			{
				...
				for (std::vector<Attachment*>::iterator it3 = (*it2)->children.begin(); it3 != (*it2)->children.end();
				     ++it3)
				{
...
void ModelControl::Update()
{
	...
	*/
	...
	for (size_t i = 0; i < model->geosets.size(); i++)
	{
		...
		data->geosetId = i;
		wxTreeItemId item = clbGeosets->AppendItem(geosetGroupsMap[mesh],
		                                           wxString::Format(
			                                           wxT("%i [%i, %i, %i]"), i, mesh, (model->geosets[i]->id % 100),
			                                           model->geosets[i]->id), -1, -1, data);
