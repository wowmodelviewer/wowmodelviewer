...
void RenderTexture::Init(int width, int height, bool fboMode)
{
	...
	else
	{
		...
		if (iCount == 0)
		{
			LOG_ERROR << "wglChoosePixelFormatARB() Failed! PixelBuffer could not find an acceptable pixel format!" <<
				glGetError();
			return;
		}
		...
		{
			LOG_ERROR <<
				"Critical error!  Out-of-Memory during PixelBuffer creation. PixelBuffer could not be created.";
			return;
		}
		else
		{
		...
		{
			LOG_ERROR <<
				"The width and height of the created PixelBuffer don't match the requirements. Image likely to come out distorted.";
			nHeight = h;
			nWidth = w;
		}
