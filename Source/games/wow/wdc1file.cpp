...
WDC1File::WDC1File(const std::string& file): WDB5File(file), m_header()
{
}
...
bool WDC1File::open()
{
  ...
  LOG_INFO << "--------------------------";
	...
	uint32 stringTableOffset = static_cast<uint32>(sizeof(header) + sizeof(field_structure) * fieldCount + recordSize * recordCount);
	...
	if ((m_header.flags & 0x04) != 0)
		copyBlockOffset += static_cast<uint32>(recordCount * 4);
  ...
  LOG_INFO << "relationshipDataOffset" << relationshipDataOffset;
    ...
    for (uint offset = 0; offset < m_header.pallet_data_size; offset+=4)
    {
      seek(palletBlockOffset + offset);
      uint32 val = 0;
      read(&val, 4);
      LOG_INFO << palletBlockOffset + offset << val;
    }
    LOG_INFO << "PALLET DATA";
#endif
		uint fieldId = 0;
	...
	for (auto it : structure->fields)
	{
		...
		for (uint i = 0; i < field->arraySize; i++)
		{
			...
			if (field->type == "text")
			{
				char* stringPtr;
				if (m_isSparseTable)
					stringPtr = reinterpret_cast<char*>(recordOffset + m_fieldStorageInfo[field->pos].field_offset_bits
						/ 8);
				else
