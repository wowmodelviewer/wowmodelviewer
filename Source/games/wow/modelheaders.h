...
struct ModelHeader
{
	...
	// 1: tilt x, 2: tilt y, 4:, 8: add BlendMaps fields in header, 16: ; 0x200000 : 24500 upgraded model format: chunked .anim files, change in the exporter reordering sequence+bone blocks before name
	...
	// Only used in models where GlobalModelFlags & 8 is true. Otherwise this field does not exist!
...
struct ModelAnimation
{
	...
	uint32 flags;
	uint16 probability;
	// This is used to determine how often the animation is played. For all animations of the same type, this adds up to 0x7FFF (32767).
	uint16 unused;
	uint32 d1;
	uint32 d2;
...
struct ModelCameraDef
{
	...
	// ( glm::vec3) How the cameras position moves. Should be 3*3 floats. (? WoW parses 36 bytes = 3*3*sizeof(float))
...
struct ModelParticleParams
{
	...
	// (UInt16)    Looks like opacity (short), Most likely they all have 3 timestamps for {start, middle, end}.
	FakeAnimationBlock sizes; // (glm::vec2)  It carries two floats per key. (x and y scale)
	int32 d[2];
	FakeAnimationBlock Intensity;
	// (UInt16) Some kind of intensity values seen: 0,16,17,32(if set to different it will have high intensity)
	...
	float unknown2[2];
	glm::vec3 Rot1; //Model Rotation 1
	glm::vec3 Rot2; //Model Rotation 2
	glm::vec3 Trans; //Model Translation
	float f2[4];
...
struct M2ParticleDef
{
	...
	// If used then it's usually 11, 12 or 13. This one is used so you can assign a color to specific particles. They loop over all
	...
	// When greater than 0, the initial velocity of the particle is (particle.position - C3Vector(0, 0, zSource)).Normalize()
