			...
			if (field->type == "text")
			{
				...
				else
				{
					stringPtr = reinterpret_cast<char*>(recordOffset + m_fieldStorageInfo[field->pos].field_offset_bits
						/ 8 + val - ((m_header.record_count - m_sectionHeader[0].record_count) * m_header.record_size));
				}
...
bool WDC3File::readFieldValue(unsigned int recordIndex, unsigned int fieldIndex, uint arrayIndex, uint arraySize,
