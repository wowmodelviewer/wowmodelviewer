...
AnimManager::AnimManager(WoWModel& m) : model(m)
{
	AnimIDSecondary = -1;
...
void AnimManager::NextFrame() // Only called by the animation controls
{
	...
	const ssize_t TimeDiff = (model.anims[id].length / 60);
	Frame += TimeDiff;
...
{
	const ssize_t id = animList[PlayIndex].AnimID;
	const ssize_t TimeDiff = (model.anims[id].length / 60) * -1;
	Frame += TimeDiff;
	ForceModelUpdate(TimeDiff);
}
...
{
	const ssize_t TimeDiff = f - Frame;
	const ssize_t id = animList[PlayIndex].AnimID;
